...
import {
  BreadcrumbItemType,
  generateItems,
} from "@/lib/constants/navigation-menus";
...
const BreadCrumb = ({ items, pathname }: IBreadcrumbProps) => {
        ...
        index === itemsArray.length - 1 ? (
          <Typography variant="muted" key={uuid()}>
            <span className="flex cursor-not-allowed items-center gap-1">
              {index === 0 && <IconLayoutDashboard className="h-4 w-4" />}{" "}
              {item.label}
            </span>
          </Typography>
          ...
          >
            <Typography variant="small">
              <span className="flex items-center gap-1">
                {index === 0 && <IconLayoutDashboard className="h-4 w-4" />}{" "}
                {item.label} <IconChevronRight className="text-brand h-4 w-4" />
              </span>
            </Typography>
          </Link>
        ) : (
